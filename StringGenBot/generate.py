...
ask_ques = "**• ذا كنت تريد تنصب ميوزك اختار بايروجرام\n\n• واذا تريد تنصب تلثيون فأختار تيرمڪس\n\n• يوجد استخرجات جلسات لي البوتٌُٕات**"
...
buttons_ques = [
    ...
    [
        InlineKeyboardButton("بايروجرام بوتٌُٕ 💎", callback_data="pyrogram_bot"),
        InlineKeyboardButton("تلثيون بوتٌُٕ 💎", callback_data="telethon_bot"),
    ],
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if api_id_msg.text == "/skip":
        ...
        try:
            ...
            return
        api_hash_msg = await bot.ask(user_id, "» حسنا قم بارسال الٍٓـ API_HASH", filters=filters.text)
        if await cancelled(api_hash_msg):
            return
        api_hash = api_hash_msg.text
    if not is_bot:
        t = "» ✔️الان ارسل رقمك مع رمز دولتك , مثال :+20123455667''"
    else:
    ...
    if not is_bot:
        await msg.reply(" انتظر سوف نرسل كود لحسابك بالتلجرام  .")
    else:
